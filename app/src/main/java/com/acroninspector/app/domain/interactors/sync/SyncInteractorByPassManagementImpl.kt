...
class SyncInteractorByPassManagementImpl(
    ...
    override fun loadData(sessionId: String): Flowable<Int> {
        ...
        entityList = listOf(
                ...
                syncRepository.loadAcronEntities(FunctionNames.DEFECTS, DefectColumns.getColumns()).toFlowable(),
                Flowable.just(R.string.defect_causes),
                syncRepository.loadAcronEntities(FunctionNames.DEFECT_CAUSES, DefectCauseColumns.getColumns()).toFlowable(),
                Flowable.just(R.string.defects_by_pos),
                syncRepository.loadAcronEntities(FunctionNames.DEFECT_RELATIONS, DefectRelationColumns.getColumns()).toFlowable()
        )
    ...
    override fun uploadData(sessionId: String): Flowable<Int> {
        ...
        entityList = listOf(
                ...
                Completable.mergeArray(
                        syncRepository.changeTaskExecutors(),
                        syncRepository.changeRouteOrderNumbers()
                ).toFlowable()
        )
        return Flowable.concat(entityList)
    }
